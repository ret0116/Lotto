...
class ConstellationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            intent.putIntegerArrayListExtra(
                "result", ArrayList(
                    LottoNumberMaker.getLottoNumbersFromHash(
                        makeConstellationString(dataPicker.month, dataPicker.dayOfMonth)
                    )
                )
            )
        ...
        textView.text = makeConstellationString(dataPicker.month, dataPicker.dayOfMonth)

        // DatePicker의 날짜가 변화하면 별자리를 보여주는 텍스트뷰로 변경
        val calendar = Calendar.getInstance()
        dataPicker.init(calendar.get(Calendar.YEAR),
